  ...
  for (t in 2:NumYears) {
    ...
    for (m in 1:12){
      ...
      for (a in 2:(maxAge.)) {
        ...
        N_at[t, a] = exp(-Mseasonal - Va[a - 1] * F*open[m]) * N_at[(t - 1), (a - 1)]
        
        # Additional calculation for group maxAge
        plusGroup = N_at[t, maxAge] + exp(-Mseasonal - Va[maxAge] * F*open[m]) * N_at[(t - 1), (maxAge)]
        
        N_at[t, maxAge] = plusGroup
        
        #Catch by cohort = number of individual age a in year t - indvid. age a captured in year t----
        
        Catch[t, a] = (((Va[a] * F*open[m]) / (Va[a] * F*open[m] + Mseasonal)) * N_at[(t), (a)] * (1 - exp(-Mseasonal - Va[a] * F*open[m])))*(Weight[a]/1000)
